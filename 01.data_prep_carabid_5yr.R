...
# grouping them in 5-year study period
# Subset by year
complete_data <- subset(complete_data, year>=1992 & year<2022)

# Grouping data by years
complete_data <- complete_data %>%
  mutate(year_group=cut(year, seq(from = 1991, to = 2022, by = 5),
...
# Exporting output
write_rds(complete_data, "data/complete_data_carabid_5yr.rds")
...
# Exporting output
write_rds(occMatrix, "data/occ_matrix_carabid_5yr.rds")
