...
visit_data <- visit_data %>%
  group_by(visit, Year, yday, site, protected_areas, builtUpAreas, forest,
           shrubs, water_bodies, agriculture) %>%
...
visitSummary <- visit_data %>%
  group_by(site, Year, protected_areas, builtUpAreas, forest,
           shrubs, water_bodies, agriculture) %>%
...
visit_data$paIndex <- as.numeric(visit_data$protected_areas)

#make response into the matrix
y <- reshape2::acast(visit_data, site ~ yearIndex ~ paIndex ~ visit,
                     value.var = "Species")
dim(y)
...
paMatrix <- expand.grid(site =siteDF$site,
                          paIndex = unique(visit_data$paIndex))

occ.covs <- list(site = as.numeric(as.factor(siteDF$site)),
                 year = reshape2::acast(yearMatrix, site ~ yearIndex, value.var = "yearIndex"),
                 pa = reshape2::acast(paMatrix, site ~ paIndex, value.var = "paIndex"))
...
det.covs <- list(year = reshape2::acast(visit_data, site ~ yearIndex ~ visit, value.var = "yearIndex"),
...
det.formula <- ~ (1|year) + nuSpecies
occ.formula <- ~ year + (1 | site) + (1 | pa)
