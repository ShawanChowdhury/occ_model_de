...
# grouping them in 10-year study period
# Subset by year
complete_data <- subset(complete_data, year>=1982 & year<2022)

# Grouping data by years
complete_data <- complete_data %>%
  mutate(year_group=cut(year, seq(from = 1981, to = 2022, by = 10),
                        labels=c("1","2","3", "4")))
...
# Exporting output
write_rds(complete_data, "data/complete_data_carabid_10yr.rds")
...
# Exporting output
write_rds(occMatrix, "data/occ_matrix_carabid_10yr.rds")
