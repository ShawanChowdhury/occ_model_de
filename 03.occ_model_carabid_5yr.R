...
doc <- "usage: 03.occ_model_carabid_5yr.R <species> <output_dir>"
opts <- docopt(doc)
...
### Get data #############################################
visit_data <- read_rds("data/complete_data_carabid_5yr.rds")
...
occMatrix <- read_rds("data/occ_matrix_carabid_5yr.rds")
...
output_file <- saveRDS(out, 
                       file = paste0("/work/chowdhus/ModelOutput/5yr/ModelOutput_", 
                                     species_name,".rds"))
...
# Exporting output
output_file <- write.csv(psiCovs, file = paste0("/work/chowdhus/ModelOutput/5yr/ModelSummary_", 
                                                species_name,".csv"))
